...
Функция ExecutePost(Запрос)
	Попытка
		ТелоЗапроса = ПрочитатьJSONИзТелаЗапроса(Запрос);
		ТекстКода = ПолучитьОбязательноеСвойство(ТелоЗапроса, "code");
		
		Выполнить(ТекстКода);
		
		ОтветДанные = Новый Структура;
		ОтветДанные.Вставить("status", "executed");
		
		Возврат СформироватьJSONОтвет(200, ОтветДанные);
	Исключение
		Возврат СформироватьТекстовыйОтветОшибки(ОписаниеОшибки());
	КонецПопытки;
КонецФункции  
...
Функция QueryPost(Запрос)
	Попытка
		ТелоЗапроса = ПрочитатьJSONИзТелаЗапроса(Запрос);
		ТекстЗапроса = ПолучитьОбязательноеСвойство(ТелоЗапроса, "query");
		
		Запрос1С = Новый Запрос(ТекстЗапроса);
		РезультатЗапроса = Запрос1С.Выполнить();
		ДанныеОтвета = СериализоватьРезультатЗапросаВJSON(РезультатЗапроса);
...
Функция ПрочитатьJSONИзТелаЗапроса(Запрос)
	
	ТелоЗапроса = Запрос.ПолучитьТелоКакСтроку(КодировкаТекста.UTF8);
	Если ПустаяСтрока(СокрЛП(ТелоЗапроса)) Тогда
		ВызватьИсключение "Тело запроса не заполнено";
	КонецЕсли;
	
	Данные = mcp_ОбщегоНазначения.JSONВСтруктуру(ТелоЗапроса);
	Если Данные = Неопределено Тогда
		ВызватьИсключение "Не удалось разобрать JSON в теле запроса";
	КонецЕсли;
...
Функция ПолучитьОбязательноеСвойство(Данные, ИмяСвойства)
	...
	ИначеЕсли ТипЗнч(Данные) = Тип("Соответствие") Тогда
		Если Данные.СодержитКлюч(ИмяСвойства) Тогда
			Возврат Данные[ИмяСвойства];
		КонецЕсли;
	КонецЕсли;
	
	ВызватьИсключение СтрШаблон("Поле '%1' не заполнено", ИмяСвойства);
	
КонецФункции

Функция СформироватьJSONОтвет(КодСостояния, ДанныеОтвета)
	
	JSONСтрока = mcp_ОбщегоНазначения.СтруктураВJSON(ДанныеОтвета);
...
Функция СформироватьТекстовыйОтветОшибки(ТекстОшибки)
...
Функция СериализоватьРезультатЗапросаВJSON(РезультатЗапроса)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРезультата Цикл
		...
		Для Каждого Колонка Из ТаблицаРезультата.Колонки Цикл
			СтрокаJSON.Вставить(Колонка.Имя, ПреобразоватьЗначениеДляJSON(СтрокаТаблицы[Колонка.Имя]));
		КонецЦикла;
...
Функция ПреобразоватьЗначениеДляJSON(Значение)
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		Для Каждого Пара Из Значение Цикл
			РезультатСтруктура.Вставить(Пара.Ключ, ПреобразоватьЗначениеДляJSON(Пара.Значение));
		КонецЦикла;
	...
	Если ТипЗначения = Тип("Соответствие") Тогда
		РезультатСтруктура = Новый Структура;
		Для Каждого Пара Из Значение Цикл
			РезультатСтруктура.Вставить(Строка(Пара.Ключ), ПреобразоватьЗначениеДляJSON(Пара.Значение));
		КонецЦикла;
		Возврат РезультатСтруктура;
	КонецЕсли;
